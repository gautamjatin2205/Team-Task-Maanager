...
export function TeamPageClient() {
  ...
  async function loadData() {
    try {
      setLoading(true);
      // Fetch dashboard data which includes team information
      const dashRes = await fetch("/api/dashboard");
      if (!dashRes.ok) throw new Error("Failed to load team data");
      
      const dashJson = await dashRes.json();
      
      // Extract team members from tasks and projects
      const membersMap = new Map<string, TeamMember>();
      
      // Collect all unique members from the data
      if (dashJson.tasksPerUser) {
        dashJson.tasksPerUser.forEach((user: any) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-2 text-gray-600">Manage team members and their assignments</p>
      </div>
      ...
      <section className="grid gap-4 grid-cols-1 sm:grid-cols-3">
        <div className="rounded-xl bg-white p-4 shadow-sm border border-gray-200">
          ...
          <div className="mt-2 text-2xl font-bold text-gray-900">{data.totalMembers}</div>
        </div>
        <div className="rounded-xl bg-white p-4 shadow-sm border border-gray-200">
          <div className="text-sm text-gray-500">Total Projects</div>
          <div className="mt-2 text-2xl font-bold text-gray-900">{data.totalProjects}</div>
        </div>
        <div className="rounded-xl bg-white p-4 shadow-sm border border-gray-200">
          <div className="text-sm text-gray-500">Total Tasks</div>
          <div className="mt-2 text-2xl font-bold text-gray-900">{data.totalTasks}</div>
        </div>
      </section>
          ...
          <table className="w-full">
            <thead className="border-b border-gray-200 bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-center text-sm font-semibold text-gray-900">Tasks</th>
                <th className="px-4 py-3 text-center text-sm font-semibold text-gray-900">Projects</th>
              </tr>
            </thead>
